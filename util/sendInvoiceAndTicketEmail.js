...
const sendInvoiceAndTicketEmail = async (recipientEmail, booking,midtransOrderId) => {
  ...
  const sendInvoiceAndTicketEmailRoundTrip = async (recipientEmail, booking,midtransOrderId) => {
    try {
      // console.log("booking file", booking);

      // const { booking,transactionId, email, finalState, discountValue } = req.body;
