    ...
    try {
        ...
        include: [
          {
            ...
            attributes: [
              'id', 'contact_name', 'contact_phone', 'contact_email', 'contact_passport_id',
              'contact_nationality', 'schedule_id', 'subschedule_id', 'gross_total',
              'gross_total_in_usd', 'exchange_rate', 'currency', 'payment_status',
              'payment_method', 'booking_date', 'total_passengers', 'adult_passengers',
              'child_passengers', 'infant_passengers', 'ticket_id', 'booking_source',
              'bank_fee','created_at','updated_at' // ✅ Include bank fee
            ],
            include: [
              {
                model: Schedule,
              ...
              },
              {
                ...
                include: [
                  ...
                  { model: Transit, as: 'TransitFrom', include: { model: Destination, as: "Destination", attributes: ["id", "name"] } },
                  { model: Transit, as: 'TransitTo', include: { model: Destination, as: "Destination", attributes: ["id", "name"] } }
                ]
              }
            ]
          }
      ...
      for (let i = 0; i < processedBookings.length; i++) {
        ...
        if (item.payment_status === 'paid') {
          runningBalance -= Number(item.gross_total);
        } else if (item.payment_status === 'invoiced') {
          runningBalance += Number(item.gross_total);
        }
        // Pasang current_balance ke item
      ...
      const totalGrossAmount = processedBookings.reduce((total, booking) => total + parseFloat(booking.gross_total), 0);
      // get the total amount paid for the booking with paymen status paid
      const totalAmountPaid = processedBookings.reduce((total, booking) => total + parseFloat(booking.amount_paid), 0);
      // get total amount (gross amount - commission)

      const totalCommissionAmount = processedBookings.reduce((total, booking) => total + parseFloat(booking.commission), 0);
      // get total amount invoiced with the payment status invoiced
      const totalAmountInvoiced = processedBookings.reduce((total, booking) => total + parseFloat(booking.amount_invoiced), 0);
     
      // tototal amount is gross total - agent comission
      const totalAmount = processedBookings.reduce((total, booking) => total + parseFloat(booking.amount), 0);
      const totalBankFee = processedBookings.reduce((total, booking) => total + parseFloat(booking.bank_fee), 0);
      // get sub total is total amount - total amount paid
      const subTotal = totalAmountPaid-totalAmountInvoiced;
  
      console.log('Total Gross Amount:', totalGrossAmount);
      console.log('Total Commission Amount:', totalCommissionAmount);
      console.log('Total Amount is gross total - agent comission:', totalAmount);
      console.log('Total Bank Fee:', totalBankFee);
      console.log('Total Amount Paid for the booking with paymen status paid:', totalAmountPaid);
